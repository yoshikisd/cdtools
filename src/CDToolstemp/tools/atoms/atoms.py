"""Contains functions to generate standard object functions for atoms

The atomic scattering factors used to derive these object functions are
pulled from E.J Kirkland, Advanced Computing in Electron Microscopy,
Appendix C. doi: 10.1007/978-1-4419-6533-2_11
...
def fParams(Z):
    # This function ruthlessly plagiarized from pyMultislicer
    fparams = {z: [0 for _ in range(0, 12)] for z in range(1, 104)}
...
def generate_atom(Z, Ksq):

    coeffs = fParams(Z)
        
    # This will be the k-space representation of the object function
    scattering_factors = np.zeros(Ksq.shape,dtype=np.complex64)
    ...
    for m in range(0, 3):
        ...
        c = a + 6
        d = b + 6
        # Keep in mind that these constants are in angstrom-related units.
        # for a; m=0,2,4 -> A^-1
        # for b; m=1,3,5 -> A^-2
        # for c; m=6,8,10 -> A
        # for d; m=7,9,11 -> A^2
        
        # This is the functional form of the parameterization
        scattering_factors += (1e10*coeffs[a])/(Ksq+1e20*coeffs[b])
        scattering_factors += 1e-10*coeffs[c]*np.exp(-1e-20*coeffs[d] * Ksq)
        
    # This is the real-space distribution
    return np.fft.ifft2(scattering_factors)
